...
public class GunService implements GunServiceInterface{
    ...
    public Gun updateGun(Gun gun) {
        ...
        gunValidator.validate(gun);
        var existingGun = gunRepository.findGunWithOrders(gun.getId());
        gun.setGun_orders(existingGun.getGun_orders());
        existingGun = gunRepository.findGunWithAccessories(gun.getId());
        gun.setGunAccessories(existingGun.getGunAccessories());
        var result = gunRepository.save(gun);
...
//        }
    ...
    /**
     * Get a gun
     * @param id the ID of the gun we are looking for
     * @return Gun
     */
    public Gun getGun(Long id) {
        log.trace("getGun - method entered: gunID={}", id);
        var gunOpt = gunRepository.findById(id);
        var result = new Gun();
        if(gunOpt.isPresent())
            result = gunOpt.get();
        log.trace("getGun: result={}", result);
        return result;
    }
    ...
    public List<Gun> getTop3SoldGuns(){
        ...
        Map<Gun, Integer> gunsNOrders = new HashMap<>();
        gunRepository.findAllWithOrders().forEach(gun -> gunsNOrders.put(gun, gun.getGun_orders().size()));

        List<Integer> sortedValues = new ArrayList<>(gunsNOrders.values());
        sortedValues.sort(Collections.reverseOrder());
        List<Integer> top3 = sortedValues.stream().limit(3).collect(Collectors.toList());
        ...
        top3.forEach(noOfSales -> {
            ...
            gunsNOrders.remove(topGun,noOfSales);
